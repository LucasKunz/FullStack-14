...
document.addEventListener('DOMContentLoaded', async () => {
  // const tokenLocal = localStorage.getItem('email')
  // const tokenSession = sessionStorage.getItem('email')
  ...
  if (emailSession) {
    ...
    const respostaApi = await buscarContatos(emailSession) // { modelo da api }
    ...
    // verifica a quantidade total de paginas para montar a quantidade certa de botões
    // console.log(respostaApi.totalPaginas)
    montarBotoes(emailSession, respostaApi.totalPaginas)
  }
...
async function buscarContatos(emailUsuarioLogado, paginaMostrada) {
  try {
    const resposta = await apiConfig.get(`/users/${emailUsuarioLogado}/contatos/listar`, {
      params: {
        page: paginaMostrada || 1,
      }
    })

    paginaAtual = paginaMostrada || 1
